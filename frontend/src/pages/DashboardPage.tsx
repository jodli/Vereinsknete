            ...
            <Card>
                <ul>
                    {setupSteps.map((step, index) => (
                        <li
                            key={index}
                            className={`py-4 ${index < setupSteps.length - 1 ? 'border-b border-gray-200' : ''
                                }`}
                        >
                            <div className="flex items-start">
                                <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${step.isComplete ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'
                                    }`}>
                                    {step.isComplete ? (
