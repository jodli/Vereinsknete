...
async fn create_session(
    ...
    let session = web::block(move || {
        session_service::create_session(&pool, session_data.into_inner())
    })
    .await?
    .map_err(|e| {
...
async fn get_sessions(
    ...
    let sessions = web::block(move || {
        session_service::get_all_sessions(&pool, Some(query.into_inner()))
    })
    .await?
    .map_err(|e| {
...
async fn get_client_sessions(
    ...
    let sessions = web::block(move || {
        session_service::get_sessions_by_client(&pool, client_id.into_inner())
    })
    .await?
    .map_err(|e| {
